...
const allPosts = ref([])
const likingPosts = ref(new Set()) // Track which posts are being liked
...
// Toggle like function for instant feedback
const toggleLike = (post, event) => {
  ...
  if (likingPosts.value.has(post.id)) {
    return // Already processing this post
  }
      ...
      router.reload({
        ...
        onSuccess: (page) => {
          if (page.props.posts && page.props.posts.data && page.props.posts.current_page === 1) {
            ...
            // Merge fresh data with existing posts, keeping pagination
            if (existingPosts.length > freshPosts.length) {
              // Update the first page posts with fresh like/comment counts
              freshPosts.forEach((freshPost, index) => {
                if (existingPosts[index]) {
                  existingPosts[index] = freshPost
                }
              })
            } else {
      ...
      <div v-for="post in allPosts" :key="post.id" class="border rounded-lg p-4 shadow-sm">
        <Link :href="`/posts/${post.id}`" class="block hover:bg-gray-50 hover:p-11 hover:rounded-lg transition-all duration-500 ease-in-out">
          ...
          <div class="flex items-center gap-4 mt-3 pt-3 border-t border-gray-100 text-sm text-gray-600">
            ...
            <button
              @click="(event) => toggleLike(post, event)"
              :disabled="likingPosts.has(post.id)"
              class="flex items-center gap-1 hover:text-red-500 transition-colors duration-200 disabled:opacity-50"
              :class="{ 'text-red-500': post.is_liked_by_user }"
            >
              <svg 
                class="w-4 h-4 transition-all duration-200" 
                :class="post.is_liked_by_user ? 'fill-current text-red-500' : 'fill-current'"
                viewBox="0 0 20 20"
              >
            ...
            <!-- Comment Count - Not clickable, just informational -->
